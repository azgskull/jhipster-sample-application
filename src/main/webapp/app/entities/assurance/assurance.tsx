...
export const Assurance = (props: IAssuranceProps) => {
  ...
  return (
    <div>
      <h2 id="assurance-heading" data-cy="AssuranceHeading">
        <Translate contentKey="jhipsterSampleApplicationApp.assurance.home.title">Assurances</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplicationApp.assurance.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplicationApp.assurance.home.createLabel">Create new Assurance</Translate>
          </Link>
        </div>
      </h2>
        ...
        {assuranceList && assuranceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.assurance.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('code')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.assurance.code">Code</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('details')}>
                  <Translate contentKey="jhipsterSampleApplicationApp.assurance.details">Details</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {assuranceList.map((assurance, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {assurance.dateDebut ? <TextFormat type="date" value={assurance.dateDebut} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>{assurance.dateFin ? <TextFormat type="date" value={assurance.dateFin} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{assurance.details}</td>
                  <td>
                    {assurance.organismeAssurance ? (
                      <Link to={`organisme-assurance/${assurance.organismeAssurance.id}`}>{assurance.organismeAssurance.nom}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${assurance.id}`} color="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationApp.assurance.home.notFound">No Assurances found</Translate>
            </div>
          )
        )}
      </div>
