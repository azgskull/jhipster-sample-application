...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_ROLE: 'role/PARTIAL_UPDATE_ROLE',
...
export default (state: RoleState = initialState, action): RoleState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_ROLE):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_ROLE):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_ROLE):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IRole> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_ROLE,
